...
- `<skills-dir>/<domain>/SKILL.md` — one skill per domain, generated with progressive
  ...
  - **Stale skills are pruned.** Every generated SKILL.md starts with a
    `<!-- Generated by pattern-learner (write-outputs). ... -->` marker line. After
    writing, `write-outputs` removes any `<skills-dir>/<domain>/` whose SKILL.md
    carries that exact marker but whose domain no longer has approved rules (renamed,
    merged, or all rejected), so an outdated skill cannot keep auto-loading. Skills
    without the marker (hand-written ones sharing the directory) are never touched.
