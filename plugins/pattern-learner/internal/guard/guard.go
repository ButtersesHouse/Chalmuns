...
const sanctioned = "Use the pattern-learner subcommands (extract-lean, verify-grounding, classify, triage, state-read/write, write-outputs, audit-format, promote) instead. If a needed capability is missing or a subcommand is broken, STOP and report it — do not write a workaround script."
